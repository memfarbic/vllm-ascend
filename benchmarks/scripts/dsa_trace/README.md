...
### 5.5 如何查看所有 tag、切换到指定 tag（以 `v0.13.0rc1` 为例）
...
#### 5.6.1 推荐做法：从 tag 创建分支再移植

建议从 tag 创建一个新分支，然后把改动 cherry-pick 过去。示例（以 `v0.13.0rc1` 为例）：

```bash
git fetch --tags
git checkout -b <your_branch_name> refs/tags/v0.13.0rc1
```

然后 cherry-pick 插桩相关的提交：
...
不同 vLLM tag 的目录结构和实现细节可能会变化，常见冲突包括：

- **attention 实现差异**：例如 `sfa_v1.py` 的 metadata 字段、是否存在 `attn_state`、`block_table` 字段名差异（`block_table` vs `block_tables`）
- **外部 KV 目录结构差异**：例如 `distributed/kvpool/*` vs `distributed/kv_transfer/*`，同名 connector 文件位置不同
- **model runner 结构差异**：`execute_model()` 的流程与变量名差异

处理原则：

- **先恢复 tag 版本的语义正确性**（保证原本能跑），再把插桩逻辑按“等价插入点”加回去
  - A/B/D：确保仍在 **topk_indices 产生后、sparse attention 前** 插入
  - step_idx 绑定：确保在每次 `execute_model()` 的“真实执行步”只递增一次
  - C：确保在实际的 `batch_get`/`transfer`/`lookup` 调用周围记录
- 遇到冲突先用 `git show refs/tags/<tag>:<path>` 对照原始 tag 的实现，再把插桩 block 适配进去

#### 5.6.4 本仓库已完成的 `v0.13.0rc1` 迁移记录（可复现命令）

下面是一次完整迁移的可复现命令序列（将 main 上的插桩改动移植到 `v0.13.0rc1` 并推送到远端）：
...
# 4) 若出现冲突：按“等价插入点”策略解决后继续
git add <resolved_files...>
git cherry-pick --continue

# 5) 移植文档提交
git cherry-pick 2efc7c44
