            ...
            if tracer.enabled:
                if forward_context.in_profile_run:
                    ...
                    if torch_dynamo is not None and getattr(torch_dynamo, "is_compiling", lambda: False)():
                        pass
                    elif tracer.decode_only and has_prefill:
                        pass
                    else:
                        ...
                        attn_state = "prefill" if has_prefill else "decode"
                        ...
                        for tok_i in range(num_tokens):
                            ...
                            if req_idx < 0 or req_idx >= len(seq_lens):
                                continue
                            request_id = (
                                batch_req_ids[req_idx]
                                if req_idx < len(batch_req_ids)
                                else f"req_idx_{req_idx}"
                            )
                            ...
                            if unique_pos_count > 0 and block_size > 0:
                                ...
                                if logical_block_idx.size > 0:
                                    logical_block_idx = np.clip(
                                        logical_block_idx, 0, row.shape[0] - 1
                                    )
                                touched_blocks = row[logical_block_idx]
                                uniq_blocks, counts = np.unique(
                                    touched_blocks, return_counts=True
                                )
                                ...
                                try:
                                    prefix_hit_tokens = int(
                                        tracer.get_prefix_hit_tokens(request_id)
                                    )
                                except Exception:
                                    prefix_hit_tokens = 0
                                prefix_cached_blocks = (
                                    int(prefix_hit_tokens // block_size)
                                    if block_size > 0
                                    else 0
                                )
                                if prefix_cached_blocks > 0:
                                    logical_uniq, logical_cnts = np.unique(
                                        logical_block_idx, return_counts=True
                                    )
                                    in_prefix = logical_uniq < prefix_cached_blocks
                                    denom = int(max(1, logical_uniq.size))
                                    dsa_prefix_intersection_ratio = float(
                                        int(in_prefix.sum()) / denom
                                    )
                                    if np.any(in_prefix):
                                        ...
                                        for b, c in zip(
                                            prefix_blocks.tolist(),
                                            prefix_counts.tolist(),
                                        ):
                                            hot_blocks.append(
                                                {
                                                    "logical_block": int(b),
